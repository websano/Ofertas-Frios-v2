...
<head>
	...
	<style> 
		...
		.item{position: absolute;bottom: 120px;left: 100px;}
		.descricao{position: absolute;top: 30px;left: 60px;color: #ffffff;font-size: 2.9em;font-family: arial;font-weight: 700;text-shadow: 5px 5px 5px black;width:790px;}	
		.preco{position: absolute;top: 165px; right: 190px; color: #ff6a00;font-size: 10em;font-family: serif;font-weight: 700;text-shadow: 5px 5px 5px black;}				
	...
	if(oci_execute($stProd)){
		while (($rowProduto = oci_fetch_assoc($stProd)) != false) {
			if($rowProduto['STATUS'] == 'ATIVO'){
				$arrayProd .= "\"carrega_oferta.php?seqproduto=".$rowProduto['SEQPRODUTO']."&loja=".$loja."&dtinicio=".$rowProduto['DTAINICIO']."&dtfinal=".$rowProduto['DTAFIM']."\",";							
			}		
		}
	}
?>
