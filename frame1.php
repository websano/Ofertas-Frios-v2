...
<head>
	...
	<style>
		...
		.container {padding:0px;margin:0px;background: url('img/fundo_ofertas.jpg') no-repeat;height:826px;background-size:768px 600px;}
		.item{position: absolute;bottom: 70px;left: 100px;}		
		.descricao{position: absolute;top: 30px;left: 40px;color: #ffffff;font-size: 2.1em;font-family: arial;font-weight: 700;text-shadow: 5px 5px 5px black;width:590px;}	
		.preco{position: absolute;top: 120px; right: 130px;color: #ff6a00;font-size: 6em;font-family: serif;font-weight: 700;text-shadow: 5px 5px 5px black;}				
		.preconormal{position: absolute;top: 100px; right: 130px;color: #fff;font-size: 1.2em;font-weight: 700;text-shadow: 5px 5px 5px black;}				
		.promocao{position: fixed;top: 250px; left: 500px;}				
		.rodape {position: absolute;top: 490px; left: 5%;color: #FFF;font-size: 1.2em;font-family: arial;font-weight: 400;text-shadow: 2px 2px 5px black;}
		.dtofertas{color:#ff6a00;}
		.precokilo{color:white;position:absolute;top:229px;left:550px;width:200px;font-family: arial;font-size:1.0em;padding:5px 10px;font-weight:600;}
	...
	if(oci_execute($stProd)){
		while (($rowProduto = oci_fetch_assoc($stProd)) != false) {
			if($rowProduto['STATUS'] == 'ATIVO'){
				$arrayProd .= "\"carrega_oferta1.php?seqproduto=".$rowProduto['SEQPRODUTO']."&loja=".$loja."&dtinicio=".$rowProduto['DTAINICIO']."&dtfinal=".$rowProduto['DTAFIM']."\",";							
			}		
		}
	}
?>
